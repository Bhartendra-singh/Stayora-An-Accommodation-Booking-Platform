...
const RoomDetails=()=>{
    ...
    const checkAvailability=async()=>{
        try {
            //check in check-in date is grater than cheeck-out date
            ...
            const {data}=await axios.post('/api/bookings/check-availability',
                ...
                if(data.isAvailable){
                   setIsAvailable(true)
                    ...
                    toast.error('Room is not avilable')
                }
            }else{
    ...
    useEffect(()=>{
        const room=rooms.find(room=>room._id.toString() === id)
        room && setRoom(room)
    ...
    return room&&(
        <div className="py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32">
            ...
            <div className="grid grid-cols-2 gap-4 lg:w-1/2 w-full">
                {room?.images?.length > 1 && room.images.map((image,index)=>(
                    <img onClick={()=>setMainImage(image)}
                     key={index} src={image} className={`w-full rounded-xl shadow-md object-cover cursor-pointer
                         ${mainImage === image && 'outline outline-2 outline-orange-500'}`} alt="Roomimage"/>
                ))}
            </div>
           </div>
           ...
           <div className="flex flex-col md:flex-row md:justify-between mt-10">
            ...
            justify-between bg-white shadow-[0px_0px_20pc_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto mt-16 max-w-6xl">
            ...
            <div className="max-w-3xl border-y border-gray-300 my-15 py-10 text-gray-500">
                <p>
                    ...
                    quoted is for two guest, at the guest slot please mark the number of guests to get the exact price for groups. 
                </p>                                      
            </div>
